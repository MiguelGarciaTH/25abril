...
public class ArticleController {
    ...
    List<ArticleRecordSentences> getArticleBySearchEntity(@PathVariable int entityId) {
        ...
        for (ArticleRecord record : records) {
            ...
            recordSentences.add(new ArticleRecordSentences(record.articleId(), record.siteId(), record.siteName(), record.score(), record.url(), record.title(), texts));
        }
    ...
    record ArticleRecordSentences(int articleId, int siteId, String siteName, int score, String url, String title,
                                  List<String> texts) {
    }
